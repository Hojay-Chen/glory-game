...
## Phase 7 Batch 7 Part 2 Flight 原语 + Zone Deploy Entity（2026-09-04，本次）

- **DDQ-B5-6 裁定落地（方案 B: 通用 Flight 原语）**: Fighter.FlightTicks 域（重力免除+高度上限 6m+到期恢复重力；快照携带）+ **击坠**（飞行中被击中→伤害封顶 1200（GDD §14.4.3）+清飞行+强制长倒地——置于受击反应后统一施加，不被浮空覆盖）+ **时长数据化**（飞行4s(+1) → FlightBaseTicks=240/FlightBonusTicks=60 同一时间域）。**触发入口保持 DDQ**——原语行为以注入式探针验证（FL01/FL02/FL03）
- **DDQ-B5-7 裁定落地（Zone = Deploy Entity 三层）**: DeployKind.Zone=7；念气罩（circle:r3.5:耐久2000）→ Zone 实体（耐久=DeployHp 2000、生命周期=act 30s→1800T、施法者 2T 动作窗解耦）；魔界之花（zone hitbox+summon）→ Zone 实体（root 效果脉冲+受火伤+50% 数据化 ZoneFireWeaknessQ）；**三层架构**: 第一层 Deploy Entity（生成/Owner/位置/生命周期/耐久/销毁/快照）+ 第二层 Zone Effect（ZonePulse 范围检测→目标过滤→contact 应用）+ 第三层技能数据（念气罩耐久/魔界之花火弱点）——全部经既有 Units 快照通道（恢复自反一致，Z01 验证）
- **Channel 独立**: 4 行 channel 未并入 Zone（施法者持续维持 vs 战场实体——生命周期不同类）；DDQ-B7-Channel 登记
- **测试 178/178**（+FL01/02/03 + Z01/Z02）；三门禁 PASS
- **踩坑**: 念龙波式 status「对敌:削攻削速」/拉拽/截脉等 13 行白名单维持（Epic 语义）；飞行探针高度 1.5m（地面攻击者判定带可达——对空 ×1.15 语义后续批次）

## DDQ-B7（Batch 7 登记，待用户裁定）

1. **Flight 触发入口**（唯一缺口——原语已建）：扫把掌握被动的飞行如何进入（跳跃延展？冲刺进入？独立键？）——裁定后 WIT_PAS_001 签名接线
2. **Channel 独立语义**（4 行: 寒冰降雨/绝对零度/六星光牢/禁法结界）：引导类施法状态机（吟唱条+打断+持续效果）——独立于 Zone/独立于普通 Act
3. **Zone 吸收语义**（念气罩「吸收一切攻击伤害」）：友方保护判定（投射物拦截？伤害转移？）——v1 Zone 实体为静默耐久实体
4. **Zone 承伤公式**: v1 简化 = mult × ownerAtk（无防御/部位/修正）——是否需要全额伤害公式
5. **拉拽/拖拽 Pull 原语**（悬磁炮吸附拖拽/勾魂 2.5m/死亡之门/束缚可拖拽）：强制位移方向+速率+碰撞交互
6. **震地波**（背摔 r2.5 二段 AOE）：落点可控+小范围后续伤害
7. **截脉四围剥夺**（ATK/攻速/移速/防御 −8s）：削弱域扩展（攻速/移速域 Sim 缺失）
8. **全异常满值**（禁忌大爆炸）：14 类异常一次施加满控制值
9. **对敌削攻削速**（念龙波打敌方分支）
10. **冻结值+50**（寒冰符控制值直接注入）
11. **打断攻击/打断吟唱**（陷阱扣/百流斩）：中断语义与 §4.3 关系
12. **分身/假身/替身**（NJA 影分身/影舞/替身术+WRK 幽魂缠绕）：SpawnDecoy 原语（ADR-0008 登记）+ 替身随机传送
13. **乱雷携带**（SPF_U_001）：手雷库存 + 全种类投掷（OQ-13 关联）

## Phase 7 Batch 7 Part 3 Pull/强制中断/Decoy/Channel（2026-09-04，本次）

- **Pull 原语**: status「拉拽:目标向自身2.5m」/「拖拽:目标向自身位移」→ ParsePullToward 提取距离 → HitResolve 设置 PullVel（指向攻击者，位移×9 击退同律）→ ④ IntegrateFighters PullTicks 窗内 IntegrateMove 强制位移 + **摩擦衰减 ×85/100（总位移=标称距离）**——勾魂 PL01 验证 2.5m 精确
- **强制中断**: status「打断攻击」/special「打断吟唱」→ ForceInterrupt → 命中即中断目标执行体（独立于伤害/霸体/§4.3）——陷阱扣 FI01 验证（root+强制中断组合）
- **Decoy**: special「假身/影分身」→ IsDecoy → 静置诱饵实体（存在期/快照全携带，BuildFromSkillDef 传递 Decoy）——影分身 DC01 验证（真身换位/瞬移语义 DDQ-B7-14 保持）
- **Channel 周期化（数据驱动通用规则）**: type=channel && hits 缺省 && iv>0 → active/iv 周期重复判定（火焰喷射 9 段/格林机枪 15 段/寒冰降雨/神佑之光）——CH01 验证多段燃烧+channel 天然打断压制（周期命中中断敌方反击）
- **DDQ-B7-5 实证**: 悬磁炮「proj:吸附拖拽」特殊弹道数值未裁定 → PL02 白名单验证（cast 无异常+拉拽数据化+unroutedHitbox 登记）——不猜弹道
- **测试 184/184**（+PL01/PL02/FI01/DC01/CH01/PL03 六探针）；三门禁 PASS
- **踩坑**: FI01 双方同 tick 起手时上挑先打断陷阱施法（§4.3 时序——先手方获胜），探针须让陷阱先手部署

## DDQ-B7 残留（Part 3 收口后：13→7 项，全部 Epic 语义）

1. **Flight 触发入口**（原语已建）
2. **Channel 独立状态机**（v1 已覆盖多段周期+可中断；吟唱条 UI/打断加成等表现层语义待定）
3. **Zone 吸收语义**（念气罩「吸收一切攻击伤害」——投射物拦截/伤害转移）
4. **Zone 承伤公式**（v1 简化 = mult × ownerAtk）
5. **Pull 弹道**（悬磁炮吸附拖拽：飞行变缓下坠+触地爆炸+拖拽窗口）
6. **震地波**（背摔 r2.5 二段 AOE）
7. **全异常满值**（禁忌大爆炸 14 类展开）
8. **截脉攻速域**（v1 三围已路由：虚弱/减速/防御剥夺待 BuffDefPct 负值接线）
9. **对敌削攻削速数值**（念龙波——已用 GDD §7.3 基准值，专属数值待补）
10. **冻结值+50 注入**（v1 冻结施加近似——控制值直接注入待定）
11. **影舞标杆转移**（NJA_U_001 分身×3+切换+标杆被杀转移）
